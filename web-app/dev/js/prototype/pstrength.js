...
initialize: function(options) {
    ...
    var o=this.options;
    var e=$(o.id);
    var txt=new Element('div',{'id':o.id+'_text'});
    var bar=new Element('div',{'id':o.id+'_bar',style:'border:1px solid white; font-size:1px; height:2px; width:0px;'}); 
    if(o.where=='before') e.insert({before:txt}).insert({before:bar});
    else e.insert({after:txt}).insert({after:bar}); 

    this.keyupEvent = this.onkeyup.bindAsEventListener(this);
    Event.observe(e,'keyup',this.keyupEvent);
},
...
onkeyup:function() {
    ...
    bar.setStyle({'backgroundColor':strColor});
    txt.innerHTML = '<span style="color:' + strColor + ';">' + strText + '</span>';
},
